...
public class ProgramPanel implements Initializable {
    ...
    private void populateAllProgramsList() {
        ...
        //For Program
        Statement forStatement = new CompoundStatement(new AssignmentStatement("v", new ConstantExpression(20)), new CompoundStatement(new ForStatement(new AssignmentStatement("v", new ConstantExpression(5)), new LessThanExpression(new VariableExpression("v"), new ConstantExpression(3)), new AssignmentStatement("v", new ArithmeticExpression(new VariableExpression("v"), new ConstantExpression(1), '+')), new ForkStatement(new CompoundStatement(new PrintStatement(new VariableExpression("v")), new AssignmentStatement("v", new ArithmeticExpression(new VariableExpression("v"), new ConstantExpression(1), '+'))))), new PrintStatement(new ArithmeticExpression(new ConstantExpression(10), new VariableExpression("v"), '*'))));

        ObservableList<Statement> programsObservable = FXCollections.observableArrayList();
        programsObservable.addAll(/*workingInMemory, file, heapStatement, threadStatement, */forStatement);
